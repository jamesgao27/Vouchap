...
import {
  ...
  getProjectTodosTree,
  getProjectByOrderId,
  getReceiptsByProjectTodoIds,
  createProjectTodo,
  updateProjectTodo,
  linkReceiptToProjectTodo,
...
export default function OrderTodosScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const filesMap = await getReceiptsByProjectTodoIds(taskIds);
  ...
  const onUploadFile = useCallback(
    async (todoId: string) => {
      try {
        ...
        const { error: linkErr } = await linkReceiptToProjectTodo(todoId, receiptId);
        if (linkErr) {
          if (Platform.OS === 'web') window.alert('Link failed: ' + linkErr.message);
          else Alert.alert('Link failed', linkErr.message);
          return;
        }
        processReceiptInBackground(imageUrl, receiptId, processedUri).catch(() => {});
        const todosTree = await getProjectTodosTree(orderId);
        setTree(todosTree);
        const taskIds = collectTaskIds(todosTree);
        const filesMap = await getReceiptsByProjectTodoIds(taskIds);
                ...
                {files.length === 0 ? (
                  ...
                  files.map((f) => (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                    >
                      <Ionicons name="document-outline" size={18} color="#6C5CE7" />
                      <Text style={styles.fileRowTitle} numberOfLines={1}>{f.name || 'File'}</Text>
