...
export default function SpaceSelectScreen() {
  ...
  const [switchModalPhase, setSwitchModalPhase] = useState<'idle' | 'switching' | 'success' | 'error'>('idle');
  ...
  const handleSelectSpace = async (spaceId: string) => {
    ...
    setSwitching(true);
    setSwitchModalPhase('switching');
    try {
      const { error } = await setCurrentSpace(spaceId);
      if (error) {
        setSwitchErrorMsg(error.message);
        setSwitchModalPhase('error');
        return;
      }
      const updatedUser = await getCurrentUser(true);
      const updatedSpace = updatedUser ? await getCurrentSpace(true) : null;
      await initializeAuthCache(updatedUser, updatedSpace);
      if (typeof window !== 'undefined') {
        setSwitchModalPhase('success');
      } else {
        router.replace('/');
      }
    } catch (error) {
      console.error('Error selecting space:', error);
      const msg = error instanceof Error ? error.message : '切换空间失败';
  ...
  return (
    <View style={styles.container}>
      ...
      {/* 切换空间：仅 Web 显示浮窗，移动端无弹窗直接路由跳转 */}
      <Modal
        visible={Platform.OS === 'web' && switchModalPhase !== 'idle'}
        animationType="fade"
        transparent
        onRequestClose={closeSwitchModal}
        statusBarTranslucent
      >
        <View style={[styles.modalOverlay, styles.refreshModalOverlay]} pointerEvents="box-none">
          <View style={styles.refreshModalCard}>
            <View style={styles.pickerHandle} />
            <View style={styles.refreshModalBody}>
              {switchModalPhase === 'switching' && (
                <>
                  <ActivityIndicator size="large" color="#6C5CE7" style={{ marginBottom: 16 }} />
                  <Text style={[styles.modalTitle, { marginBottom: 8 }]}>Switching space...</Text>
                  <Text style={styles.refreshModalSubtitle}>Please wait</Text>
                </>
              )}
              {switchModalPhase === 'success' && (
                <>
                  <Text style={[styles.modalTitle, { marginBottom: 24 }]}>Space switched successfully</Text>
                  <TouchableOpacity
                    style={[styles.modalButton, { marginTop: 0 }]}
                    onPress={handleRefreshPage}
                    activeOpacity={0.8}
                  >
                    <Text style={styles.modalButtonText}>Refresh page</Text>
                  </TouchableOpacity>
                </>
              )}
              {switchModalPhase === 'error' && (
                <>
                  <Text style={[styles.modalTitle, { marginBottom: 12, color: '#C0392B' }]}>Switch failed</Text>
                  <Text style={[styles.refreshModalSubtitle, { marginBottom: 24 }]}>{switchErrorMsg}</Text>
                  <TouchableOpacity
                    style={[styles.modalButton, { marginTop: 0 }]}
