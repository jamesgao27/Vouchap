-- Pure clients cannot SELECT firm.skus; preview RPCs must match how they may SELECT firm.orders.
-- Centralize the same predicate as firm_orders_select (see 20260323154000) and use it for:
--   - order-scoped SKU preview RPCs (avoid drift vs duplicated OR branches)
--   - _client_can_preview_sku fallback when p_firm_client_id/token are null: any order for this SKU in this firm
--     that the user may SELECT → allow template preview (same intent as seeing that engagement).
SET search_path = public, firm;

--------------------------------------------------------------------------------
-- Mirrors firm_orders_select USING(...) — keep in sync when that policy changes.
--------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION firm.auth_user_can_select_order(
  p_user_id uuid,
  p_order_id uuid
)
RETURNS boolean
...
COMMENT ON FUNCTION firm.auth_user_can_select_order(uuid, uuid) IS
  'True if p_user_id may SELECT this order under firm_orders_select (can_access_order OR client_space membership).';

--------------------------------------------------------------------------------
-- _client_can_preview_sku: allow SKU template when user may SELECT any order with this SKU in this firm.
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE o.sku_id = p_sku_id
      ...
      AND firm.auth_user_can_select_order(v_uid, o.id)
  ) THEN
    RETURN true;
  END IF;
...
COMMENT ON FUNCTION firm._client_can_preview_sku(uuid, uuid, text) IS
  'Preview: firm member, firm_client/token paths, SELECTable order for SKU, or same-email invitee rows.';

--------------------------------------------------------------------------------
-- Order-scoped preview: firm_orders_select OR pending invitee (order not bound to space yet).
--------------------------------------------------------------------------------
...
BEGIN
  ...
  WHERE o.id = p_order_id
    ...
    AND (
      firm.auth_user_can_select_order(v_uid, p_order_id)
      OR (
...
COMMENT ON FUNCTION public.get_firm_sku_preview_for_order_client(uuid) IS
  'SKU header when firm.auth_user_can_select_order OR pending invitee email for this order.';
...
COMMENT ON FUNCTION public.get_firm_sku_items_preview_for_order_client(uuid) IS
  'sku_items for order SKU; same gate as get_firm_sku_preview_for_order_client.';
