-- preset_sku_items：多前置依赖（与 firm.sku_items.depends_on_ids 对齐）
-- 依赖：请先应用 20260321143000_sku_items_depends_on_ids.sql（firm.sku_items 需有 depends_on_ids）
ALTER TABLE firm.preset_sku_items
  ADD COLUMN IF NOT EXISTS depends_on_ids uuid[] NOT NULL DEFAULT '{}';

COMMENT ON COLUMN firm.preset_sku_items.depends_on_ids IS 'Predecessor preset_sku_item ids (multi). depends_on_id should match first element when set.';

UPDATE firm.preset_sku_items
SET depends_on_ids = ARRAY[depends_on_id]::uuid[]
WHERE depends_on_id IS NOT NULL;

UPDATE firm.preset_sku_items
SET depends_on_id = depends_on_ids[1]
WHERE cardinality(depends_on_ids) > 0
  AND depends_on_id IS DISTINCT FROM depends_on_ids[1];

-- 复制到 firm 时映射整组 depends_on_ids → sku_items（与单 depends_on_id 回填兼容）
...
BEGIN
  ...
  -- 回填 depends_on_ids + depends_on_id（与客户端 updateSkuItemDependsOn 一致）
  FOR r IN
...
COMMENT ON FUNCTION firm.apply_preset_skus_to_firm(UUID) IS
  '将 preset_skus + preset_sku_items 复制到指定 firm；幂等（已有 skus 则跳过）。创建 firm 时由 create_space_with_user 服务端调用，客户端也可兜底调用。Step 2 将 preset 的 depends_on_ids 映射到 firm.sku_items（depends_on_id 为首项）。';
